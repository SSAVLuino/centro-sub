...
import { FileText, CheckCircle, XCircle, AlertTriangle, Users, Search, ExternalLink, ChevronDown, ChevronUp } from "lucide-react"
...
export default function CertificatiClient({
  sociAttivi,
  certificati,
  stats,
}: {
  ...
  // Mappa socio -> certificato più recente
  ...
  }, [sociAttivi, certPerSocio, search, filter, today, in30days])
  ...
  const statsCards = [
    ...
    },
    {
      ...
      value: stats.conCertificato,
      sub: `${stats.senzaCertificato} senza`,
    ...
    {
      label: "Scaduti / Assenti",
      value: stats.scaduti + stats.senzaCertificato,
                ...
                rows.map(({ socio, cert, status }) => (
                  <tr key={socio.id} className="hover:bg-secondary/30 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-sm text-muted-foreground">
                      {cert ? formatDate(cert["Data visita"]) : "—"}
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      {cert?.["Data scadenza"] ? (
                        <span className={`text-sm font-medium ${
                          ...
                          status === "in_scadenza" ? "text-amber-600" :
                          "text-emerald-600"
                        }`}>
